...
/*   Updated: 2020/03/11 02:38:35 by jko              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	return_all(char **remainder, char **line, int read_size)
{
	if (read_size < 0)
		return (-1);
	if (*remainder != 0)
...
static int	split_and_save(char **remainder, char **line, char *cut_addr)
{
	char	*temp;
	size_t	len;
...
int	get_next_line(int fd, char **line)
{
	...
	while ((cut_addr = ft_strchr(info[fd], '\n')) == 0)
	{
		if ((read_size = read(fd, buf, BUFFER_SIZE)) <= 0)
			return (return_all(&info[fd], line, read_size));
		buf[read_size] = 0;
		if (join(&info[fd], buf, read_size) == -1)
			return (-1);
	}
	return (split_and_save(&info[fd], line, cut_addr));
}
